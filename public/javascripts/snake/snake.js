...
function createSnake() {
    ...
    var amountOfPartsByDefault = window.score;
    ...
    updateLocalStorageResult(getSnakeSize());
}
...
function checkNewPosition() {
    ...
    for (var z = 1; z < snake.length; z++) {
        ...
        if (head.position().top === part.position().top && head.position().left === part.position().left) {
            showInfoAlert('<h4>Game over!</h4><br/>Your Score: <b>' + snake.length + '</b><br/><br/><a href="#" onclick="location.reload();">Start new game!</a><br/><br/>');
            updateLocalStorageResult(getSnakeSize(), true);
